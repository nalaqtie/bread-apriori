...
def return_item_df(item_antecedents):
    ...
    matched = data.loc[data["antecedents"] == item_antecedents, "consequents"]
    if matched.empty:
        return []

    val = matched.iloc[0]

    # If parse_list produced a comma-separated string, split into items
    if isinstance(val, str):
        if "," in val:
            return [v.strip() for v in val.split(",") if v.strip()]
        return [val]

    # If it's already an iterable (list/set/frozenset), convert to list
    try:
        return list(val)
    except Exception:
        return [val]
...
if type(data) != type("No Result"):
    ...
    consequents = return_item_df(item)
    if consequents:
        st.success(f"Jika Konsumen Membeli **({item})**, maka membeli **{consequents[0]}** secara bersamaan")
    else:
        st.info("Tidak ada rekomendasi untuk item ini.")
